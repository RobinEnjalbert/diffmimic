...
class HumanoidMimic(env.Env):
    ...
    def step(self,
        ...
        state.metrics.update(step_index=step_index,
    ...
    def _get_obs(self,
                 qp: QP,
                 step_index: jp.ndarray) -> jp.ndarray:
        """
        ...
        if self.obs_type == 'timestamp':
            ...
            target_pos, target_rot, target_vel, target_ang = (target_qp.pos[:-1],
                                                              target_qp.rot[:-1],
                                                              target_qp.vel[:-1],
                                                              target_qp.ang[:-1])
            ...
            obs = jp.concatenate([pos.reshape(-1), rot.reshape(-1), vel.reshape(-1), ang.reshape(-1),
                                  target_pos.reshape(-1), target_rot_6d.reshape(-1), target_vel.reshape(-1),
                                  target_ang.reshape(-1)], axis=-1)
        else:
