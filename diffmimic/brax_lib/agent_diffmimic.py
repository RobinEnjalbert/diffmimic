...
def train(environment: envs.Env,
    ...
    logging.info(f'Device count: {jax.device_count()}, process count: {process_count} (id {process_id}), '
                 f'local device count: {local_device_count}, devices to be used count: {local_devices_to_use}')
    ...
    apg_network = network_factory(env.observation_size,
                                  env.action_size,
    ...
    def env_step(carry: Tuple[envs.State, PRNGKey],
                 step_index: int,
                 policy: types.Policy):

        env_state, _key = carry
        _key, key_sample = jax.random.split(_key)
        actions = policy(env_state.obs, key_sample)[0]
        n_state = env.step(env_state, actions)
        if truncation_length is not None:
            n_state = jax.lax.cond(jnp.mod(step_index + 1, truncation_length) == 0.,
                                   jax.lax.stop_gradient,
                                   lambda x: x,
                                   n_state)

        return (n_state, _key), (n_state.reward, env_state.obs, n_state.metrics)

    def loss(_policy_params, normalizer_params, _key):

        key_reset, key_scan = jax.random.split(_key)
        env_state = env.reset(jax.random.split(key_reset, num_envs // process_count))
        _f = functools.partial(env_step,
                               policy=make_policy((normalizer_params, _policy_params)))
        (rewards, obs, _metrics) = jax.lax.scan(_f,
                                                (env_state, key_scan),
                                                (jnp.array(range(episode_length // action_repeat))))[1]
        return -jnp.mean(rewards), (rewards, obs, _metrics)
    ...
    def clip_by_global_norm(updates):
        ...
        return jax.tree_util.tree_map(lambda t: jnp.where(trigger, t, (t / g_norm) * max_gradient_norm),
                                      updates)

    def training_epoch(_training_state: TrainingState,
                       _key: PRNGKey):

        _key, key_grad = jax.random.split(_key)
        grad_raw, (rewards, obs, _metrics) = loss_grad(_training_state.policy_params,
                                                       _training_state.normalizer_params,
                                                       key_grad)
        grad = clip_by_global_norm(grad_raw)
        grad = jax.lax.pmean(grad, axis_name='i')
        grad_raw = jax.lax.pmean(grad_raw, axis_name='i')
        params_update, optimizer_state = optimizer.update(grad, _training_state.optimizer_state)
        _policy_params = optax.apply_updates(_training_state.policy_params,
                                             params_update)

        normalizer_params = running_statistics.update(_training_state.normalizer_params,
                                                      obs,
                                                      pmap_axis_name=_PMAP_AXIS_NAME)
        _metrics = {'grad_norm': optax.global_norm(grad_raw),
                    'params_norm': optax.global_norm(_policy_params),
                    'loss': -1 * rewards,
                    **_metrics}
        return TrainingState(optimizer_state=optimizer_state,
                             normalizer_params=normalizer_params,
                             policy_params=_policy_params), _metrics
    ...
    def training_epoch_with_timing(_training_state: TrainingState,
                                   _key: PRNGKey) -> Tuple[TrainingState, Metrics]:

        nonlocal training_wall_time
        t = time.time()
        (_training_state, _metrics) = training_epoch(_training_state, _key)
        _metrics = jax.tree_util.tree_map(jnp.mean, _metrics)
        jax.tree_util.tree_map(lambda x: x.block_until_ready(), _metrics)

        epoch_training_time = time.time() - t
        training_wall_time += epoch_training_time
        sps = (episode_length * num_envs) / epoch_training_time
        _metrics = {'training/sps': sps,
                    'training/wall_time': training_wall_time,
                    **{f'training/{name}': value for name, value in _metrics.items()}}
        return _training_state, _metrics
    ...
    training_state = TrainingState(optimizer_state=optimizer.init(policy_params),
                                   policy_params=policy_params,
                                   normalizer_params=running_statistics.init_state(
                                       specs.Array((env.observation_size,), jnp.float32)))
    training_state = jax.device_put_replicated(training_state,
    ...
    if process_id == 0 and num_evals > 1:
        metrics, _ = evaluator.run_evaluation(_unpmap((training_state.normalizer_params,
    ...
    for it in range(num_evals_after_init):
        ...
        if process_id == 0:
            # Run evals.
            metrics, qp_list = evaluator.run_evaluation(_unpmap((training_state.normalizer_params,
                                                                 training_state.policy_params)),
                                                        training_metrics)
            best_pose_error = min(metrics['eval/episode_pose_error'], best_pose_error)
            metrics['eval/best_pose_error'] = best_pose_error
